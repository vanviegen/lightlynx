...
1.  **Automatic**: Copy the contents of [lightlynx-api.js](build.frontend/extensions/lightlynx-api.js) and add it as a new "External Extension" in the Zigbee2MQTT web interface (Settings -> External Extensions).
2.  **Manual**: Copy [lightlynx-api.js](build.frontend/extensions/lightlynx-api.js) directly to your Zigbee2MQTT `data/extension` directory and restart Zigbee2MQTT.
...
- **Extensions**: Custom Zigbee2MQTT extensions are used to optimize state payloads, handle authentication, and implement automation logic directly on the Z2M server:
    - **lightlynx-api**: Provides a WebSocket API used by the web app. It supports multi-user permissions, remote access control, and stores its configuration in `data/lightlynx.json`.
